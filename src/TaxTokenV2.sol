...
import { ITreasury, IUniswapV2Factory, IUniswapV2Router01, IERC20 } from "./interfaces/ERC20.sol";
...
contract TaxTokenV2 {
    ...
    address public TaxTokenV1;    /// @dev The contract used for TaxTokenV1.
    ...
    constructor(
        ...
        balances[address(this)] = totalSupplyInput * 10**_decimals;     // Initial liquidity, allocated entirely to this contract (escrowed, only redeemable via swapV1()).
    ...
    /// @notice User can swap their V1 tokens (after approval) for V2 tokens here.
    ///         This burns the old tokens, and transfers to mgs.sender the new tokens.
    /// @dev    The initial supply of V2 tokens is held in escrow in this contract.
    function swapV1() external returns (bool success) {
        
        uint256 balanceTaxTokenV1 = IERC20(TaxTokenV1).balanceOf(msg.sender);

        IERC20(TaxTokenV1).transferFrom(msg.sender, address(this), balanceTaxTokenV1);

        balances[address(this)] -= balanceTaxTokenV1;
        balances[msg.sender] += balanceTaxTokenV1;
    }
